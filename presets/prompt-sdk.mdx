...
async function customerSupport(question: string) {
  ...
  const prompt = murnitur.formatPrompt(preset.PresetPrompts[0].prompts, { question });
...
async function main() {
  ...
  const response = await customerSupport("Do you offer discount on Hardware items?");
